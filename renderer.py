...
Adapted from the vispy demo: https://github.com/vispy/vispy/blob/master/examples/demo/gloo/cloud.py
"""
...
class Canvas(app.Canvas):
    ...
    def __init__(self):
        ...
        data = np.zeros(n, [('a_position', np.float32, 3),
                            ...
                            ('a_size', np.float32, 1)])
        data['a_position'] = points #0.45 * np.random.randn(n, 3)
        data['a_bg_color'] = np.random.uniform(0.85, 1.00, (n, 4))
        data['a_fg_color'] = colors#np.random.uniform(0.85, 1.00, (n, 4))#colors
    ...
    def on_key_press(self, event):
        ...
        self.view = translate((self.translate_x, self.translate_y, -self.translate))  
        self.model = np.dot(rotate(self.theta, (1, 0, 0)),
                            np.dot(rotate(self.phi, (0, 0, 1)),
                                   rotate(self.eta, (0, 1, 0))))
        self.program['u_model'] = self.model
        self.program['u_view'] = self.view
        self.update()     
    ...
    def apply_zoom(self):
        ...
        self.projection = perspective(100.0, self.size[0]/float(self.size[1]), 10.0, 1000.0)
